...
const Chat = () => {
  ...
  const sendMessage = (e) => {
    e.preventDefault()
    if (inputValue.trim() === "") {
      document.getElementById('emptyMessageModal').showModal()
      return
    }
    const msg = {
      text: inputValue,
  ...
  return (
    <div className='flex flex-col h-screen'>
      ...
      <div className='w-full p-5 bg-base-300 flex items-center justify-between'>
        ...
        <div>
          <button className="btn btn-soft btn-success" onClick={() => document.getElementById('my_modal_5').showModal()}>
            <IoCall />
          </button>
      ...
      {/* ❗️ Bo‘sh xabar modali */}
      <dialog id="emptyMessageModal" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          <h3 className="font-bold text-lg">Xabar yuborilmadi</h3>
          <p className="py-4">Ubu narsa yoz</p>
          <div className="modal-action">
            <form method="dialog">
